...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    bool pickup_achieved;

    nh.getParam("pickup_achieved", pickup_achieved);

    if (pickup_achieved == false)
    {
        if ( (current_position.x < map_pickup_position["x"] + 0.25 && current_position.x > map_pickup_position["x"] - 0.25 ) && (current_position.y < map_pickup_position["y"] + 0.25 && current_position.y > map_pickup_position["y"] - 0.25 ) )
        {
    ...
    bool dropoff_achieved;

    nh.getParam("dropoff_achieved", dropoff_achieved);

    if (pickup_achieved == true && dropoff_achieved == true)
    {
            marker.ns = "basic_shapes";
            marker.id = 0;
